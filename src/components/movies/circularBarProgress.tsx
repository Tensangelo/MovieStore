...
// Definimos el tipo para la dirección
const Direction = {
    CLOCKWISE: -1,
    ANTI_CLOCKWISE: 1,
} as const; // `as const` asegura que las claves sean literales y no cadenas generales
...
// Componente de la barra de progreso circular
const CircularProgressBar = ({
    maxValue = MAX_VALUE,
    selectedValue = 0,
    radius = DEFAULT_RADIUS,
    strokeWidth = DEFAULT_RADIUS / 14,
    label = '',
    ...
    // Cálculo del pie (segmento circular)
    const generatePie = (value: number): string => {
    ...
    // Cálculo del valor del pie
    const calculatePieValue = (numberOfBars: number): number => {
    ...
    const renderPie = (i: number) => {
        const DIRECTION: DirectionType = anticlockwise ? Direction.ANTI_CLOCKWISE : Direction.CLOCKWISE;
